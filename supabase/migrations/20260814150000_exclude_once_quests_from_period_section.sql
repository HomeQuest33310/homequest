-- One-time quests must disappear after completion. Only active daily and
-- weekly quests can be shown as "Terminées pour cette période".
...
begin
  ...
  from (
    ...
    where q.kingdom_id = p_kingdom_id and q.status = 'active'
  ) available;
  return v_result;
end;
$function$;

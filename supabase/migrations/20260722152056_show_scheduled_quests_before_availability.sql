-- Scheduled quests stay visible in the catalogue. Their assignment action
-- remains disabled by the client and by self_assign_quest until available.
...
begin
  ...
  from (
    ...
    where quest.kingdom_id = p_kingdom_id
      ...
      and (
        quest.frequency in ('daily', 'weekly')
        or coalesce(quest.available_from, quest.created_at) > now()
        or public.homequest_quest_is_available_for_member(quest.id, v_member.id)
        or exists (
          select 1

-- Include the actual gains in approval notifications for regular quests.
  ...
  if tg_op = 'INSERT' and new.status = 'pending' then
    ...
    insert into public.guardian_notifications(
      ...
      case when new.status = 'approved'
        then 'completion_approved' else 'completion_rejected' end,
      case when new.status = 'approved'
        then 'Mission validée' else 'Mission à revoir' end,
      case when new.status = 'approved' then
        'Votre mission « ' || v_quest.title ||
        ' » a été validée. Gains personnels : +' || v_quest.xp_reward ||
        ' XP, +' || v_quest.gold_reward || ' or et +' ||
        v_quest.boss_damage || ' dégâts au boss.'
      else
        'Votre mission « ' || v_quest.title || ' » doit être revue.'
      end
    );
  end if;
  return new;
